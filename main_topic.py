...
parser.add_argument('--save', type=str,  default='model_topic.pt',
    ...
    if args.preprocess=='delta':
        ...
        for j in line:
            if '00'<j<'99':
                topic_idx = int(j)
                break
    utt2spk_train.append(topic_idx)
    line = i.replace('\n','').split(' ')
    line.pop(0)
    if topic_idx not in topic:
        topic[topic_idx] = [0]*len(corpus.dictionary)
        speaker_doc[topic_idx] = []
    temp = []
    for j in line:
        if j not in corpus.dictionary.word2idx:
            topic[topic_idx][corpus.dictionary.word2idx['<unk>']] += 1
            all_unigram[corpus.dictionary.word2idx['<unk>']] += 1
            temp.append(corpus.dictionary.word2idx['<unk>'])
        else:
            topic[topic_idx][corpus.dictionary.word2idx[j]] += 1
            all_unigram[corpus.dictionary.word2idx[j]] += 1
            temp.append(corpus.dictionary.word2idx[j])
    speaker_doc[topic_idx]+=[temp]
all_unigram = np.array(all_unigram)/float(sum(all_unigram))

# import SM
# for i in topic:
#     c = SM.collection(all_unigram)
#     allPtList, lambdaList = c.training(100,topic[i])
...
#         topic[i][terms[j]] = allPtList[j][1]


for i in topic:
    topic[i] = np.array(topic[i])/float(sum(topic[i]))
    # topic[i] = topic[i]*2-np.array(all_unigram)
    ...
    if adapation:
        ...
        for a in range(args.batch_size):
            for b in range(seq_len):
                if int(data[b][a])!=0:
                    zero_padding[b][a] = torch.cuda.FloatTensor(topic[utt2spk[batch_step*a+i/args.bptt]])
