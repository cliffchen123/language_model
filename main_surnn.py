...
model = model_surnn.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied)
if args.cuda:
    model.cuda()
...
def get_batch(source, i, adapation=False, evaluation=False):
    ...
    if adapation:
        zero_padding = torch.cat([source[i+2:i+seq_len],torch.cuda.LongTensor(2,args.batch_size).zero_()],0)
    futureword1 = Variable(zero_padding)
    if adapation:
        zero_padding = torch.cat([source[i+3:i+seq_len],torch.cuda.LongTensor(3,args.batch_size).zero_()],0)
    ...
    return data, target, futureword1, futureword2
...
def evaluate(data_source):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        data, targets, futureword1, futureword2 = get_batch(data_source, i, evaluation=True)
...
def train():
    ...
    model.train()
    # for p in model.future_dense1.parameters():
    #     p.requires_grad=False
    # for p in model.future_dense2.parameters():
    #     p.requires_grad=False
    # for p in model.future_dense3.parameters():
    #     p.requires_grad=False        
    total_loss = 0
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        data, targets, futureword1, futureword2 = get_batch(train_data, i, adapation=True)
