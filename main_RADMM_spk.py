...
if args.stage<=2:
    ...
    try:
        for epoch in range(1, args.epochs+1):
            ...
            # Save the model if the validation loss is the best we've seen so far.
            if not best_val_loss or val_loss < best_val_loss:
                with open(args.save_folder+'/BACKGROUND_model.pt', 'wb') as f:
                    torch.save(model, f)
                best_val_loss = val_loss
            else:
                # Anneal the learning rate if no improvement has been seen in the validation dataset.
    ...
    # Load the best saved model.
    with open(args.save_folder+'/BACKGROUND_model.pt', 'rb') as f:
        model = torch.load(f)
...
if args.stage<=3:
    ...
    if args.preprocess=='ami':
        experts = ['MEO069','MIO066','MEO082','MEO020','MIO005','MEE094','MEE095','FEE088','FEE087','MIO031','MTD030ID','MIO024','MIO100','MIO078','MIO097','MIO020','MIO106']
            for i in splitdata.keys():
                if i not in experts:
                    splitdata.pop(i)
    elif args.preprocess=='delta_new':
        splitdata.pop('ALANLEE')
        splitdata.pop('ANNEHO')
        splitdata.pop('DAVIDZBLIN')
        splitdata.pop('ELIYANG')
        splitdata.pop('GINYZHONG')
        splitdata.pop('IHENTSAI')
        splitdata.pop('IRENECHEN')
        splitdata.pop('PATRICKHUANG')
        splitdata.pop('SILENCE')
        splitdata.pop('TOMMYSHEU')
    ...
    model = model_mixer.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, len(model_expert), args.dropout, args.tied)
    model.encoder.weight = model_expert[model_expert.keys()[0]].encoder.weight
    model.decoder.weight = model_expert[model_expert.keys()[0]].decoder.weight
